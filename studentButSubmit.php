...
$studentID = mysql_real_escape_string($_GET["studid"]);
//$studentname = mysql_real_escape_string($_GET["studentname"]);
//$studentage = mysql_real_escape_string($_GET["studentage"]);
$query ="SELECT names.id, names.name, names.age, names.timeoutpre, names.finishpic, names.finishmovie, teachers.teachername, gender.type FROM names INNER JOIN teachers ON names.teacher=teachers.id INNER JOIN gender ON names.gender=gender.id WHERE names.teacher ='$tID' && names.id = '$studentID'";
